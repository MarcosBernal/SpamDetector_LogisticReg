...
# !!!! USE THESE IF YOU WANT TO USE THE PARAMS I USED IN RESULT SECTION!!!!
...
while (epoch < n_epochs and modelsAndFoldsLeft > 0):
    ...
    # step 1) compute hypothesis with the current weights and append them to the original rdd
    hypothesis_CV = trainRdd_byModelAndFold\
    .map(lambda m_f__Xy: mxl.append_hypothesis(m_f__Xy, W_CV))\
    .cache() # <<<<< important to cache!
    

    # step 2) compute the total cost for the new hypothesis for every model m and every fold f
    ...
    # step 3) 
    # - for every model and fold vector, create n new for n features
...
# Plot for every model and fold the learning curves
fig, ax = plt.subplots(5,4)
for m in range(len(ax)):
    for f in range(len(ax[0])):
        if (f == 0):
            ax[m][f].set_ylabel('Cost')
        if (m == 4):
            ax[m][f].set_xlabel('Epochs')
        
        ax[m][f].plot(costsHistory_CV[m][f])         
